...
if __name__ == "__main__":
    ...
    print(all_index_to_remove)

    print("track one win: ", len(df2.query("winner == @win")))
    print("track one two: ", len(df2.query("winner == @two")))
    print("track one tree: ", len(df2.query("winner == @tree")))
    print("track one four: ", len(df2.query("winner == @four")))
    print("track one five: ", len(df2.query("winner == @five")))
    print("track one six: ", len(df2.query("winner == @sixe")))

    #df2 = df2[(df2.winner > 1 & df2.winner < 8)]
    ...
    X_train_tf,X_test_tf,y_train_tf,y_test_tf = train_test_split(X, y_tf, test_size = 0.15 ,random_state = 0)
    model_tf = Sequential([
    ...
    model_tf.fit(X_train_tf, y_train_tf, epochs=10, batch_size=64, shuffle=False)
